function setup() {
  ...
  function source(lang) {
    ...
    $.post(
      `/lab/source/${url}/${qid}`,
      { sourceCode: editor.getValue(), lang: pre },
      function (data, status) {
        $("#lang").data("pre", $("#lang").val());

        if (!lang) lang = "";
        $.get(`/lab/source/${url}/${qid}`, { lang: lang }, function (
          data,
          status
        ) {
          if (lang) {
      ...
      $.post(
        ...
        function (data, status) {
          if (document.getElementById("custom_i").checked == true) {
            $(".custom").remove();
            $("#opbox").append(
              '<div class="card custom" style="width:650px;"><label><b>Custom TestCase</b></label><hr style="width:25%;margin-top:0rem;"><label><b>Input</b></label><textarea readonly="readonly" class="bg-col" style="font-family: Source Code Pro, monospace;width:100%;border:none;height:auto;">' +
                document.getElementById("custom_input").value +
                '</textarea ><br><label><b>Output</b></label><textarea readonly="readonly" class="bg-col" style="font-family: Source Code Pro, monospace;width:100%;border:none;height:125px;">' +
                data[0].output +
                "</textarea>"
            );
            ...
            for (let i = 0; i < data.length; i++) {
              if (data[i].id == 3) {
                $("#color" + (i + 1) + "").removeClass(
                  "red-cross yellow-exclamation"
                );
                $("#color" + (i + 1) + "").addClass("green-tick");
                $("#icon" + (i + 1) + "").removeClass(
                  "fad fa-times fad fa-exclamation-triangle"
                );
                $("#icon" + (i + 1) + "").addClass("fad fa-check");
              } else if (data[i].id == 4) {
                $("#color" + (i + 1) + "").removeClass(
                  "green-tick yellow-exclamation"
                );
                $("#color" + (i + 1) + "").addClass("red-cross");
                $("#icon" + (i + 1) + "").removeClass(
                  "fad fa-check fad fa-exclamation-triangle"
                );
            ...
            for (let i = 0; i < $("#card .sa").length; i++) {
              document.getElementById("message" + (i + 1) + "").innerHTML =
                data[i].description;
              document.getElementById("output" + (i + 1) + "").value =
                data[i].output;
              $("#input" + (i + 1) + "").html(
                document.getElementById("sam_i" + i + "").innerHTML
              );
              $("#expectedOutput" + (i + 1) + "").html(
                document.getElementById("sam_o" + i + "").innerHTML
              );
            }
    ...
    $.post(
      ...
      function (data, status) {
        ...
        for (let i = 0; i < data.length; i++) {
          ...
          let t1 = document.createTextNode(data[i].description);
          message.appendChild(t1);
          document.getElementById("myTr" + (i + 1) + "").appendChild(message);

          // let points = document.createElement("TD");
          // let t2 = document.createTextNode(data[i].points);
          // points.appendChild(t2);
...
$(document).ready(function () {
  ...
  $("#button").click(function () {
    ...
    if (count == 0) {
      ...
      for (var i = 1; i <= $("#card .sa").length; i++) {
        if (i == 1) {
          $("#Output").append(
            '<li class="nav-item"><a class="nav-link active" id="color' +
              i +
          ...
          $("#tab-content").append(
            '<div id="tab' +
              i +
              '" class="container tab-pane active"><br><label><b>Message</b></label><br><p style="font-family: Source Sans Pro, sans-serif;font-size:18px;" id="message' +
              i +
              '"></p><label><b>Input</b></label><div class="bg-col" id="input' +
              i +
              '" readonly="readonly" style="white-space:pre-wrap;font-family: Source Code Pro, monospace;width:100%;border:none;height:auto;"></div><label><b>Output</b></label><textarea class="bg-col" id="output' +
              i +
              '" readonly="readonly" style="font-family: Source Code Pro, monospace;width:100%;border:none;height:110px;"></textarea><label><b>Expected Output</b></label><div class="bg-col" id="expectedOutput' +
              i +
              '" readonly="readonly" style="white-space:pre-wrap;font-family: Source Code Pro, monospace;width:100%;border:none;height:auto;"></div></div>'
          );
        } else {
          $("#Output").append(
            '<li class="nav-item"><a class="nav-link" id="color' +
              i +
              '" data-toggle="tab" href="#tab' +
              i +
              '">Sample ' +
          ...
          $("#tab-content").append(
            '<div id="tab' +
              i +
              '" class="container tab-pane fade"><br><label><b>Message</b></label><br><p style="font-family: Source Sans Pro, sans-serif;font-size:18px;" id="message' +
              i +
              '"></p><label><b>Input</b></label><div class="bg-col" id="input' +
              i +
              '" readonly="readonly" style="white-space:pre-wrap;font-family: Source Code Pro, monospace;width:100%;border:none;height:auto;"></div><label><b>Output</b></label><textarea class="bg-col" id="output' +
  ...
  $("#button_submit").click(function () {
    ...
    if (count1 == 0) {
      $("#opbox").append(
        '<div name="out" id="out" class="container mt-3 SUBMIT_BOX" style="background:white;border-radius:4px;width:550px;box-shadow: 0 4px 8px 0 rgba(0, 0, 0, 0.2), 0 6px 20px 0 rgba(0, 0, 0, 0.19);text-align: left;left: 20px;padding: 10px;margin: 20px;position: relative;"><div id="TestCases"></div></div>'
      );
      count1 = count1 + 1;
    }
